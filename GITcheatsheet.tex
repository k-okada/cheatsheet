...
\>\> \texttt{\$ sudo apt-get install ruby}\\
...
\>Use https protocol instead of ssh for hub remote command.\\
\>\> \texttt{\$ git config --global hub.protocol https}\\
...
\href{http://wiki.ros.org/}{roswiki}(wiki.ros.org), ask
...
\> \texttt{\$ git checkout -b new\_features}\\
Write code, check diffs.\\
\> \texttt{\$ git diff [--color-words]}\\
If you added new file, add them to repository.\\
\> \texttt{\$ git add [file]}\\
Check the current status.\\
\> \texttt{\$ git status}\\
Commit the changes with descriptable message, no fix/update/hoge/fuga.\\
...
Even after above commands, you have unknown change in 'git status'.\\
...
\includegraphics[angle=90,width=.95\columnwidth]{willow_p1_02s.jpg}
\end{center}
